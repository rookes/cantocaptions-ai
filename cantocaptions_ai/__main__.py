...
from cantocaptions_ai.utils.output import (LANGUAGES, TO_LANGUAGE_CODE, optional_float,
...
def cli():
    ...
    parser.add_argument("--audio_start", type=float, default=None, help="seconds of audio to skip before processing")
    parser.add_argument("--audio_end", type=float, default=None, help="seconds of audio to cut from the ending")

    parser.add_argument("--revise_subtitles", type=str, default=None, help="a subtitle file to use for base transcription. Subtitles will be retimed")

    parser.add_argument("--model", default="Qwen3-ASR", help="name of the model to use")
    ...
    parser.add_argument("--debug_dir", type=str, default=None, help="if set, write intermediate stage data (audio segments and JSON manifests) to this directory for debugging and replay")
    parser.add_argument("--load_debug_dir", type=str, default=None, help="load intermediate stage data from a previous --debug_dir run; completed stages are skipped and their results are loaded instead")
    ...
    parser.add_argument("--log_file", type=str, default=None, help="redirect third-party stdout/stderr to this file; cantocaptions_ai log messages are written to both terminal and file")
    ...
    # vad params
    parser.add_argument("--vad_method", type=str, default="pyannote", choices=["pyannote"], help="VAD method to be used")
    parser.add_argument("--vad_onset", type=float, default=0.500, help="Onset threshold for VAD (see pyannote.audio), reduce this if speech is not being detected")
    parser.add_argument("--vad_offset", type=float, default=0.363, help="Offset threshold for VAD (see pyannote.audio), reduce this if speech is not being detected.")
    parser.add_argument("--chunk_size", type=int, default=30, help="Chunk size for merging VAD segments. Default is 30, reduce this if the chunk is too long.")

    # vocal isolation params
    parser.add_argument("--vocal_isolation_method", type=str, default="mbroformer", choices=["none", "mbroformer"], help="vocal isolation method to be used")

    # ensemble / LLM correction params
    parser.add_argument("--ensemble_model", type=str, default="none", choices=["none", "whisper"], help="second ASR model for ensemble correction; 'whisper' runs alvanlii/whisper-small-cantonese via faster-whisper alongside the primary model (requires pip install 'cantocaptions_ai[ensemble]')")
    parser.add_argument("--llm_correction", action="store_true", help="run LLM-based per-segment particle correction and full-document name normalization after transcription")
    parser.add_argument("--llm_model", type=str, default="Qwen/Qwen3-4B", help="HuggingFace model ID for LLM correction (used with --llm_correction)")
    parser.add_argument("--llm_model_dir", type=str, default=None, help="local path to LLM weights; uses HF cache if not set")
    ...
    parser.add_argument("--verify_speakers", action="store_true", help="Apply speaker verification to split out segments where speakers don't match")
    ...
    parser.add_argument("--max_line_width", type=optional_int, default=18, help="(not possible with --no_align) the maximum number of characters in a line before breaking the line")
    parser.add_argument("--max_line_count", type=optional_int, default=1, help="(not possible with --no_align) the maximum number of lines in a segment")
    ...
    parser.add_argument("--print_progress", type=str2bool, default=True, help="if True, display stage progress bars and a timing summary; also enables per-batch progress in transcribe() and align() methods")
