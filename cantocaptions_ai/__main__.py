...
def build_parser() -> argparse.ArgumentParser:
    ...
    transcript_grp.add_argument("--realign", type=str, default=argparse.SUPPRESS, metavar="TRANSCRIPT", help="transcript to put on this recording's timeline: a line-delimited text file, or a subtitle (SRT/VTT) that already has timings. One cue per line; the line breaks are the authoritative cue boundaries, so punctuation does not split or merge them, and a line break *inside* a cue is kept (it is usually a change of speaker). Skips ASR by default. See --realign_mode for what happens to an input's existing timings.")
    transcript_grp.add_argument("--realign_mode", type=str, default=argparse.SUPPRESS, choices=["auto", "transcript", "sync", "adjust"], help="what to do with the timings a --realign input already has. 'transcript' discards them and places every line from scratch. 'sync' finds the confident lines acoustically, fits the simplest transform between the subtitle's timeline and this recording's (offsets, cuts, insertions and speed changes), and maps every cue through it, so the subtitle's own proportions survive exactly. 'adjust' fits the same transform and then re-times each cue from the audio within --realign_adjust_tolerance of it. 'auto' (the default) picks 'transcript' for a bare transcript and 'sync' for a subtitle with timings. 'sync' and 'adjust' preserve the input text rather than cleaning it, since it is already a finished subtitle.")
    transcript_grp.add_argument("--realign_max_scale", type=float, default=argparse.SUPPRESS, help="how far the fitted transform's speed may differ from 1.0 (0.25 = up to a quarter faster or slower). A common cause of a large value is a frame-rate conversion, e.g. a PAL broadcast running 4.27%% fast against its 23.976 fps master. Beyond this bound the fit is refused rather than clamped, since a clamped scale is a number the fit does not believe.")
    transcript_grp.add_argument("--realign_cut_policy", type=str, default=argparse.SUPPRESS, choices=["drop", "keep"], help="what to do with cues the fitted transform says this recording has no audio for, e.g. a scene the broadcast cut. 'drop' leaves them out of the output and reports them. 'keep' collapses them onto the cut point, flagged, so nothing the user wrote is silently lost. Either way every affected cue is listed in the run summary and in realign/transform.json.")
    transcript_grp.add_argument("--realign_normalize", type=str2bool, default=argparse.SUPPRESS, help="if True, fold the input's punctuation into the forms the aligner can use before anything else sees it: a halfwidth , . ? ! ; sitting beside Chinese text becomes its fullwidth form, and any ellipsis becomes a single U+2026. Punctuation and whitespace only -- never the words. Set False to have the text reach the subtitle exactly as written, which costs alignment accuracy: a halfwidth mark is in neither the align vocabulary nor the pause tokens, so it is dropped and the pause it stands for goes unmodelled. Punctuation inside an English clause is left alone either way.")
    transcript_grp.add_argument("--realign_adjust_tolerance", type=float, default=argparse.SUPPRESS, metavar="SECONDS", help="--realign_mode adjust only: how far forced alignment may move a cue from where the fitted transform put it before the transform wins instead. Widened automatically when the transform only fits its anchors loosely.")
    transcript_grp.add_argument("--realign_sync_anchor_density", type=float, default=argparse.SUPPRESS, metavar="PER_MINUTE", help="--realign_mode sync only: roughly how many anchors per minute of the file get to set a cue's start directly, once thinned by residual against the fitted transform. Every other cue is interpolated between whichever of those survive nearby. Trusting every anchor individually exposes every cue to that anchor's own acoustic search -- a confidently-wrong anchor (a repeated phrase confusing CTC, a stylised reading) scores as well as a correct one, so raising --realign_min_score cannot fix it; thinning against the fitted transform's own residual can.")
    transcript_grp.add_argument("--realign_anchor", type=str, default=argparse.SUPPRESS, choices=["acoustic", "asr"], help="how --realign finds where each line sits. 'acoustic' slides a free-end Viterbi over the audio with no ASR pass (fast, and the default). 'asr' transcribes first and matches the two character streams. 'asr' costs a full ASR pass and no longer wins on accuracy on any fixture measured -- on the whole of Police Story 2, scored against hand-measured lines, acoustic placed 15 of 17 within half a second against asr's 3 -- so it is kept only because a text match can leave a line unmatched where forced alignment must place it somewhere.")
    ...
    diarize_grp.add_argument("--diarize", action="store_true", default=argparse.SUPPRESS, help="identify speakers after alignment, so cue assembly never merges lines from different speakers")
    diarize_grp.add_argument("--min_speakers", default=argparse.SUPPRESS, type=int, help="minimum number of speakers in the audio file")
    diarize_grp.add_argument("--max_speakers", default=argparse.SUPPRESS, type=int, help="maximum number of speakers in the audio file")
    ...
    diarize_grp.add_argument("--diarize_scope", default=argparse.SUPPRESS, type=str, choices=["segment", "file"], help="diarize each VAD segment separately (speakers are labelled per segment and only compared within one), or the whole file at once (globally consistent speakers, but one clustering error affects the whole episode)")
    diarize_grp.add_argument("--diarize_batch_size", default=argparse.SUPPRESS, type=int, help="chunks per diarization forward pass (default 4). The model config's own value of 32 needs ~10.7 GB and silently pages to host RAM on most cards; raise this only after measuring the peak with --log_level debug")
    diarize_grp.add_argument("--speaker_confidence", default=argparse.SUPPRESS, type=float, metavar="SHARE", help="share of a subtitle's diarized time one speaker must hold before it is attributed to them; lower is more willing to label (and so to split cues)")
